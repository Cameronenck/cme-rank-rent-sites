...
function initReviewsCarousel() {
  ...
  function renderStars(rating) {
    if (Icons) {
      ...
      for (let i = 0; i < 5; i++) {
        if (i < rating) {
          html += `<span class="icon-svg icon-sm" style="color:#f4b400" aria-hidden="true">${Icons.starFilled}</span>`;
        } else {
...
document.addEventListener('submit', (e) => {
  ...
  if (form.tagName === 'FORM') {
    const name = form.getAttribute('name') || 'unknown';
    ALT_Track.formSubmit(name);
  }
});
